...
export async function getExchanges(tickers: { ticker: string; url: string }[]) {
    ...
    const results = await Promise.all(tickers.map(async ({ ticker, url }) => {
...
export async function getPrice(
    { ticker, exchange }: { ticker: string; exchange: string },
) {
...
const PriceVarSchema = z.object({
    fields: z.array(FieldSchema).optional(),
}).passthrough();

const SymbolFaqSchema = z.object({
    variables: z.object({
...
export function extractPriceFromHtml(
    ...
    const scripts = $('script[type="application/prs.init-data+json"]')
        .toArray();
    if (scripts.length > 0) {
        for (const node of scripts) {
            ...
            const layers = safe.data as Record<
                string,
            ...
            for (const layer of Object.values(layers)) {
                ...
                if (Array.isArray(fields)) {
                    const close = fields.find((f) =>
                        f?.name === "daily_bar_close"
                    );
    ...
    const m = /js-symbol-last["'\s>][\s\S]*?<span>([0-9][0-9,\.]+)<\/span>/i
        .exec(html);
    if (m && m[1]) {
...
function findPriceInUnknown(root: unknown): number | undefined {
    ...
    const TradeLike = z.object({
        trade: z.object({ price: z.number() }).passthrough(),
    }).passthrough();

    const PriceVarsLike = z.object({
        variables: z.object({
